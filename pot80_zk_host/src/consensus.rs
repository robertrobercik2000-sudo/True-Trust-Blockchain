...
fn qmul(a: Q, b: Q) -> Q {
    ...
    (z >> 32).min(u128::from(u64::MAX)) as u64
}
#[inline]
...
impl TrustParams {
    #[inline] pub fn decay(&self, t: Q) -> Q { qmul(t, self.alpha_q) }
    #[inline] pub fn reward(&self, t: Q) -> Q { qclamp01(qadd(t, self.beta_q)) }
    #[inline] pub fn step(&self, t: Q) -> Q { self.reward(self.decay(t)) }
}
...
impl TrustState {
    #[inline] pub fn get(&self, id: &NodeId, init: Q) -> Q { *self.map.get(id).unwrap_or(&init) }
    #[inline] pub fn set(&mut self, id: NodeId, val: Q) { self.map.insert(id, val); }
    #[inline] pub fn apply_block_reward(&mut self, who: &NodeId, p: TrustParams) {
...
impl Registry {
    ...
    #[inline] pub fn is_active(&self, who: &NodeId, min_bond: u64) -> bool {
        self.map.get(who).map(|e| e.active && e.stake >= min_bond).unwrap_or(false)
    }
    #[inline] pub fn stake(&self, who: &NodeId) -> u64 {
        self.map.get(who).map(|e| e.stake).unwrap_or(0)
    }
    #[inline] pub fn stake_mut(&mut self, who: &NodeId) -> Option<&mut u64> {
...
impl EpochSnapshot {
    ...
    #[inline] pub fn stake_q_of(&self, who: &NodeId) -> Q { *self.stake_q.get(who).unwrap_or(&0) }
    #[inline] pub fn trust_q_of(&self, who: &NodeId) -> Q { *self.trust_q_at_snapshot.get(who).unwrap_or(&0) }
...
fn merkle_root(leaves: &[[u8; 32]]) -> [u8; 32] {
    ...
    while layer.len() > 1 {
        ...
        for i in (0..layer.len()).step_by(2) {
            if i + 1 < layer.len() { next.push(merkle_parent(&layer[i], &layer[i + 1])); }
            else { next.push(merkle_parent(&layer[i], &layer[i])); }
        }
        layer = next;
    }
    layer[0]
...
fn merkle_build_proof(leaves: &[[u8; 32]], leaf_idx: u64) -> MerkleProof {
    ...
    while layer.len() > 1 {
        let mut next = Vec::with_capacity((layer.len() + 1) / 2);
        for i in (0..layer.len()).step_by(2) {
            let (a, b) = if i + 1 < layer.len() { (layer[i], layer[i + 1]) } else { (layer[i], layer[i]) };
...
pub fn verify_merkle(proof: &MerkleProof, leaf: [u8; 32], root: [u8; 32]) -> bool {
    let mut acc = leaf; let mut idx = proof.leaf_index;
    for sib in &proof.siblings {
        acc = if (idx & 1) == 0 { merkle_parent(&acc, sib) } else { merkle_parent(sib, &acc) };
        idx >>= 1;
    }
    acc == root
}
...
impl RandaoBeacon {
    pub fn new(slash_noreveal_bps: u32, genesis_beacon: [u8; 32]) -> Self {
        Self { epochs: HashMap::new(), prev_beacon: genesis_beacon, slash_noreveal_bps }
    }
    #[inline]
    ...
    pub fn finalize_epoch(&mut self, epoch: u64) -> ([u8; 32], Vec<NodeId>) {
        ...
        for (who, r) in &revealed { mix = mix_hash(&mix, who, r); }
        e.beacon = mix;
        e.finalized = true;

        // brakujące
        let mut missing = Vec::new();
        for who in e.commits.keys() { if !e.reveals.contains_key(who) { missing.push(*who); } }
    ...
    pub fn value(&self, epoch: u64, slot: u64) -> [u8; 32] {
        ...
        let base = match self.epochs.get(&epoch) {
            Some(e) if e.seed != [0u8; 32] => e.seed,
            _ => self.prev_beacon,
        };
        let mut h = Sha256::new();
...
#[derive(Clone, Copy, Debug)]
pub struct PotParams { pub trust: TrustParams, pub lambda_q: Q, pub min_bond: u64, pub slash_noreveal_bps: u32 }
...
pub fn verify_leader_and_update_trust(
    ...
    let p_q = prob_threshold_q(params.lambda_q, wit.stake_q, wit.trust_q, epoch_snap.sum_weights_q);
...
// Zostawiamy starą funkcję `verify_leader_and_update_trust` dla zgodności; ta wersja używa
// `WeightWitnessV1` i `EpochSnapshot::verify_witness`, bez jawnego MerkleProof w API.
pub fn verify_leader_with_witness(
    ...
    // Zweryfikuj kompaktowy świadek względem root w snapshotcie (spójność Merkle i indeksu)
    if !epoch_snap.verify_witness(wit) { return None; }

    // Próg prawdopodobieństwa na podstawie stake_q i trust_q ze świadka
    let p_q = prob_threshold_q(params.lambda_q, wit.stake_q, wit.trust_q, epoch_snap.sum_weights_q);

    // Losowanie slotu na beaconie RANDAO
    let b = beacon.value(epoch, slot);
    let y = elig_hash(&b, slot, &wit.who);
    if y > bound_u64(p_q) { return None; }
    ...
    // Nagroda trustu po akceptacji nagłówka
    trust_state.apply_block_reward(&wit.who, params.trust);
    Some(weight)
}
...
pub fn detect_equivocation(proposals: &[Proposal]) -> bool {
    ...
    for p in proposals {
        if p.slot != slot || p.who != who { return false; }
        set.insert(p.header_hash);
    }
    set.len() > 1
}

pub fn slash_equivocation(reg: &mut Registry, trust: &mut TrustState, who: &NodeId, tp: TrustParams, penalty_bps: u32) {
    trust.set(*who, tp.init_q);
    if let Some(st) = reg.stake_mut(who) { *st = slash_bps(*st, penalty_bps); }
}

#[inline]
fn slash_bps(stake: u64, bps: u32) -> u64 {
    let cut = (stake as u128 * bps as u128) / 10_000u128;
    stake.saturating_sub(cut as u64)
}
...
pub fn finalize_epoch_and_slash(
    ...
    for who in missing {
        if let Some(st) = registry.stake_mut(&who) { *st = slash_bps(*st, beacon.slash_noreveal_bps); }
        trust.set(who, tp.init_q);
    }
    val
}
...
mod tests {
    ...
    #[test]
    fn prob_monotone() {
        let p1 = super::prob_threshold_q(q_from_ratio(11,10), q_from_ratio(1,10), q_from_ratio(1,1), q_from_ratio(1,1));
        let p2 = super::prob_threshold_q(q_from_ratio(11,10), q_from_ratio(2,10), q_from_ratio(1,1), q_from_ratio(1,1));
        assert!(p2 >= p1);
    }
    ...
    fn randao_commit_reveal() {
        let genesis = [7u8; 32];
        let mut b = RandaoBeacon::new(500, genesis);
        let e = 1u64; let who = nid(1);
        let r = [9u8; 32]; let c = RandaoBeacon::commit_hash(e, &who, &r);
        b.commit(e, who, c);
        ...
        // beacon.value() uses stable seed (prev_beacon at epoch start), not new beacon
        let v0 = b.value(e, 0);
        let mut h = Sha256::new();
        h.update(b"RANDAO.slot.v1");
        h.update(&e.to_le_bytes());
        h.update(&0u64.to_le_bytes());
        h.update(genesis);  // use genesis (prev_beacon), not val (new beacon)
        let mut exp = [0u8; 32];
        exp.copy_from_slice(&h.finalize()[..32]);
        assert_eq!(v0, exp, "value should use stable seed from epoch start");
        
        // new beacon becomes prev_beacon for next epoch
        assert_eq!(b.prev_beacon, val, "beacon should advance prev_beacon");
    }

    #[test]
    fn snapshot_deterministic_root() {
        let mut reg = Registry::default();
        let tp = TrustParams { alpha_q: q_from_basis_points(9900), beta_q: q_from_basis_points(100), init_q: q_from_basis_points(1000) };
        let mut ts = TrustState::default();
        let a = nid(1); let b = nid(2); let c = nid(3);
        reg.insert(a, 100, true); reg.insert(b, 50, true); reg.insert(c, 150, true);
        ts.set(a, q_from_basis_points(5000)); ts.set(b, q_from_basis_points(9000)); ts.set(c, q_from_basis_points(1000));
    ...
    fn weights_ratio_uses_u128_den() {
        let mut reg = Registry::default();
        let tp = TrustParams { alpha_q: ONE_Q, beta_q: 0, init_q: ONE_Q };
        let mut ts = TrustState::default();
        let a = [1u8; 32]; let b = [2u8; 32];
        reg.insert(a, u64::MAX, true);
        reg.insert(b, u64::MAX - 1, true);
        ts.set(a, ONE_Q); ts.set(b, ONE_Q);
