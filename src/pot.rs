...
impl TrustParams {
    ...
    #[inline]
    pub fn decay(&self, t: Q) -> Q {
        qmul(t, self.alpha_q)
    }

    #[inline]
    pub fn reward(&self, t: Q) -> Q {
        qclamp01(qadd(t, self.beta_q))
    }

    #[inline]
    pub fn step(&self, t: Q) -> Q {
        self.reward(self.decay(t))
    }
}
...
#[derive(Default)]
pub struct Registry {
    pub map: HashMap<NodeId, RegEntry>,
}
...
impl Registry {
    ...
    pub fn is_active(&self, who: &NodeId, min_bond: u64) -> bool {
        self.map
            .get(who)
            .map(|e| e.active && e.stake >= min_bond)
            .unwrap_or(false)
    }
...
impl EpochSnapshot {
    pub fn build(
        ...
        for (who, e) in &reg.map {
            if !(e.active && e.stake >= min_bond) {
                continue;
            }
            let sq = if total == 0 {
                0
            } else {
                q_from_ratio128(e.stake as u128, total)
            };
        ...
        let sum_weights_q = entries.iter().fold(0u64, |acc, e| {
            acc.saturating_add(qmul(e.stake_q, e.trust_q))
        });
...
impl RandaoBeacon {
    ...
    pub fn commit_hash(epoch: u64, who: &NodeId, r: &[u8; 32]) -> [u8; 32] {
        kmac256_hash(b"RANDAO.commit.v1", &[&epoch.to_le_bytes(), who, r])
    }
    ...
    pub fn reveal(&mut self, epoch: u64, who: NodeId, r: [u8; 32]) -> bool {
        ...
        match e.commits.get(&who) {
            None => false,
            Some(&c) => {
                if Self::commit_hash(epoch, &who, &r) != c {
                    return false;
                }
    ...
    pub fn finalize_epoch(&mut self, epoch: u64) -> ([u8; 32], Vec<NodeId>) {
        ...
        let revealed: BTreeMap<NodeId, [u8; 32]> =
            e.reveals.iter().map(|(k, v)| (*k, *v)).collect();
    ...
    pub fn value(&self, epoch: u64, slot: u64) -> [u8; 32] {
        ...
        kmac256_hash(
            b"RANDAO.slot.v1",
            &[&epoch.to_le_bytes(), &slot.to_le_bytes(), &base],
        )
    }
}
...
fn verify_leader_common(
    ...
    if !reg.is_active(who, params.min_bond) {
        return None;
    }
    if epoch != epoch_snap.epoch {
        return None;
    }
    if epoch_snap.sum_weights_q == 0 {
        return None;
    }
...
pub fn verify_leader_and_update_trust(
    ...
    // Verify Merkle proof
    if wit.weights_root != epoch_snap.weights_root {
        return None;
    }
...
pub fn verify_leader_with_witness(
    ...
    if !epoch_snap.verify_witness(wit) {
        return None;
    }
...
mod tests {
    ...
    fn randao_commit_reveal() {
        ...
        let v0 = b.value(e, 0);
        let exp = kmac256_hash(
            b"RANDAO.slot.v1",
            &[&e.to_le_bytes(), &0u64.to_le_bytes(), &val],
        );
        assert_eq!(v0, exp);
    }
    ...
    fn randao_value_stable_before_after_finalize() {
        ...
        assert_eq!(
            v_pre, v_post,
            "sortition seed must be stable across finalize"
        );
    }
