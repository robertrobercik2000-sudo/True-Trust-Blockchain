...
impl RandaoBeacon {
    ...
    pub fn commit(&mut self, epoch: u64, who: NodeId, c: [u8; 32]) {
        ...
        if e.seed.is_none() {
            e.seed = Some(self.prev_beacon);
        }
        e.commits.insert(who, c);
    }
    ...
    pub fn value(&self, epoch: u64, slot: u64) -> [u8; 32] {
        ...
        let base = self.epochs.get(&epoch)
            .and_then(|e| e.seed)
            .unwrap_or(self.prev_beacon);
        let mut h = Sha256::new();
...
mod tests {
    ...
    fn randao_commit_reveal() {
        ...
        assert_eq!(miss.len(), 0); assert_ne!(val, [0u8; 32]);
        let v0 = b.value(e, 0);
        let mut h = Sha256::new();
        h.update(b"RANDAO.slot.v1"); h.update(&e.to_le_bytes()); h.update(&0u64.to_le_bytes()); h.update(val);
        let mut exp = [0u8; 32];
