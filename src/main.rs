...
struct WalletSecretPayloadV3 {
    ...
    falcon_sk_bytes: Option<Vec<u8>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    falcon_pk_bytes: Option<Vec<u8>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    mlkem_sk_bytes: Option<Vec<u8>>,
    #[serde(skip_serializing_if = "Option::is_none")]
    mlkem_pk_bytes: Option<Vec<u8>>,
}
...
impl Keyset {
    fn from_payload_v3(p: &WalletSecretPayloadV3) -> Result<Self> {
        ...
        let (falcon_sk, falcon_pk) = match (&p.falcon_sk_bytes, &p.falcon_pk_bytes) {
            (Some(sk), Some(pk)) => (
                Some(
                    falcon512::SecretKey::from_bytes(sk)
                        .map_err(|_| anyhow!("Falcon SK invalid"))?,
                ),
                Some(
                    falcon512::PublicKey::from_bytes(pk)
...
fn bech32_addr(scan_pk: &X25519Public, spend_pk: &Ed25519Public) -> Result<String> {
    use bech32::{encode, ToBase32, Variant::Bech32m};
...
impl PepperProvider for OsLocalPepper {
    fn get(&self, wallet_id: &[u8; 16]) -> Result<Zeroizing<Vec<u8>>> {
        ...
        Ok(Zeroizing::new(pepper.to_vec()))
    }
}
...
fn derive_kdf_key(password: &str, hdr: &KdfHeader, pepper: &[u8]) -> [u8; 32] {
    match &hdr.kind {
        ...
        KdfKind::Argon2idV1 {
            ...
            let mut out = [0u8; 32];
            argon
                .hash_password_into(password.as_bytes(), salt32, &mut out)
                .expect("argon2");
...
fn encrypt_wallet<T: Serialize>(
    ...
    match hdr.aead {
        ...
        AeadKind::XChaCha20 => {
            ...
            let nonce = Nonce24::from_slice(&nonce_bytes);
            Ok(cipher
                .encrypt(
                    nonce,
...
fn decrypt_wallet_v3(
    ...
    let payload: WalletSecretPayloadV3 = bincode::options()
        .with_limit(WALLET_MAX_SIZE as u64)
...
fn load_keyset(path: PathBuf) -> Result<(Keyset, WalletHeader)> {
    ...
    let secret = decrypt_wallet_v3(&wf.enc, password.as_str(), &wf.header)?;
...
fn shards_recover(paths: &[PathBuf]) -> Result<[u8; 32]> {
    ...
    let (wallet_id, m, n) = (shards[0].0.wallet_id, shards[0].0.m, shards[0].0.n);
    for (hdr, _) in &shards {
        ensure!(
            hdr.wallet_id == wallet_id && hdr.m == m && hdr.n == n,
            "shard set mismatch"
        );
    }

    let mut recovered: Vec<(u8, Vec<u8>)> = Vec::new();
    for (hdr, ct) in shards {
        let pt = if hdr.has_pw {
            let pw = Zeroizing::new(prompt_password(format!(
                "Password for shard #{}: ",
                hdr.idx
            ))?);
            shard_mask(&ct, pw.as_str(), &hdr.salt32)
        } else {
            ct
        };
        recovered.push((hdr.idx, pt));
    }

    let sharks = Sharks(m);
    let shares_iter = recovered.into_iter().map(|(idx, bytes)| {
        Share::try_from(bytes.as_slice())
            .map(|share| (idx, share))
            .map_err(|e| anyhow!("share parse error: {}", e))
    });
    let shares_vec: Vec<(u8, Share)> = shares_iter.collect::<Result<_, _>>()?;

    let secret = sharks
        .recover(shares_vec.iter().map(|(_, sh)| sh))
...
fn cmd_wallet_init(
    ...
    let (falcon_sk_bytes, falcon_pk_bytes, mlkem_sk_bytes, mlkem_pk_bytes) = if quantum {
        let (pq_falcon_pk, pq_falcon_sk) = falcon512::keypair();
        let (pq_mlkem_pk, pq_mlkem_sk) = mlkem::keypair();
        (
            Some(pq_falcon_sk.as_bytes().to_vec()),
            Some(pq_falcon_pk.as_bytes().to_vec()),
            Some(pq_mlkem_sk.as_bytes().to_vec()),
            Some(pq_mlkem_pk.as_bytes().to_vec()),
        )
    } else {
        (None, None, None, None)
    };
...
fn cmd_wallet_export(path: PathBuf, secret: bool, out: Option<PathBuf>) -> Result<()> {
    ...
    if secret {
        let out_path = out.ok_or_else(|| anyhow!("secret export requires --out <file>"))?;
        ...
        let json = if wf.header.quantum_enabled {
            format!(
        ...
        eprintln!("🔒 secrets written → {}", out_path.display());
    } else {
