...
use chacha20poly1305::{aead::Aead as AeadCha, aead::KeyInit as KeyInitCha, XChaCha20Poly1305, XNonce as Nonce24};
...
struct KdfHeader {
    kind: KdfKind,
    info: String, // e.g., "TT-KDF.v4.argon2id..."
}
...
impl PepperProvider for OsLocalPepper {
    fn get(&self, wallet_id: &[u8;16]) -> Result<Zeroizing<Vec<u8>>> {
        ...
        #[cfg(unix)]
        {
            ...
            match OpenOptions::new().create_new(true).write(true).mode(0o600).open(&path) {
                Ok(mut f) => {
                    ...
                    f.write_all(&p)?;
                    f.sync_all()?;
                    return Ok(Zeroizing::new(p.to_vec()));
                }
                Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
                    // File exists, read it
                }
                Err(e) => return Err(e).with_context(|| format!("create pepper {}", path.display())),
            }
        }
        #[cfg(not(unix))]
        {
        ...
        let v = fs::read(&path)?;
        ensure!(v.len() == 32, "pepper file size invalid: expected 32 bytes, got {}", v.len());
        Ok(Zeroizing::new(v))
    }
}
...
/// Fixed: safer error handling, doesn't delete original file on error
...
struct ShardHeader {
    version: u32,             // 1
    scheme: &'static str,     // "shamir-gf256"
    wallet_id: [u8;16],
    m: u8,
    n: u8,
    idx: u8,                  // 1..n
    salt32: [u8;32],          // for masking share/mac
    info: String,             // "TT-SHARD.v1"
    has_pw: bool,             // whether password-masked
}
...
fn seal_share(wallet_id: [u8;16], idx: u8, m: u8, n: u8, share: &[u8], salt32: [u8;32], pw_opt: Option<&str>) -> Result<ShardFile> {
    ...
    let hdr = ShardHeader {
        version: 1, scheme: "shamir-gf256", wallet_id, m, n, idx, salt32, info: "TT-SHARD.v1".into(), has_pw
    };
...
fn shards_create(master32: [u8;32], wallet_id: [u8;16], m: u8, n: u8, pw_opt: Option<String>) -> Result<Vec<ShardFile>> {
        ...
        "invalid m-of-n scheme: m={} must be >= {}, n={} must be >= m and <= {}", 
        m, SHAMIR_MIN_M, n, SHAMIR_MAX_N);
    ...
    for (i, sh) in shares.into_iter().enumerate() {
        ...
        let sf = seal_share(wallet_id, (i+1) as u8, m, n, sh.as_ref(), salt32, pw_opt.as_deref())?;
        out.push(sf);
    }
    Ok(out)
}
...
fn shards_recover(paths: &[PathBuf]) -> Result<[u8;32]> {
    ...
    let secret = sharks.recover(shares_iter)
        .map_err(|e| anyhow!("Shamir recovery failed: {}. Ensure you have at least {} valid shards.", e, m))?;
    let mut out=[0u8;32]; 
...
/* =========================================================================================
 * Wallet header creation (refactored common code)
 * ====================================================================================== */
...
fn create_wallet_header(
    ...
    let kdf = if use_argon2 {
        ...
        KdfHeader { 
            ...
            info: format!("TT-KDF.v4.argon2id.t{}.m{}MiB.l{}", ARGON2_TIME_COST, ARGON2_MEM_KIB/1024, ARGON2_LANES) 
        }
    } else {
        let mut salt32 = [0u8; 32]; OsRng.fill_bytes(&mut salt32);
    ...
    let wallet_id = wallet_id.unwrap_or_else(|| {
        let mut id = [0u8;16]; OsRng.fill_bytes(&mut id); id
    });
...
fn prompt_and_validate_password() -> Result<Zeroizing<String>> {
    let pw1 = Zeroizing::new(prompt_password(format!("New password (min {} chars): ", MIN_PASSWORD_LEN))?);
    ensure!(pw1.len() >= MIN_PASSWORD_LEN, 
        "password too short: need at least {} characters, got {}", MIN_PASSWORD_LEN, pw1.len());
...
fn cmd_wallet_init(path: PathBuf, use_argon2: bool, aead_flag: AeadFlag, pepper_flag: PepperFlag, pad_block: u16) -> Result<()> {
    ...
    let pw = prompt_and_validate_password()?;
    let hdr = create_wallet_header(use_argon2, aead_flag, pepper_flag, pad_block, None)?;
...
fn cmd_wallet_rekey(path: PathBuf, use_argon2: bool, aead_flag: AeadFlag, pepper_flag: PepperFlag, pad_block: u16) -> Result<()> {
    ...
    let pw = prompt_and_validate_password()?;
    let hdr = create_wallet_header(use_argon2, aead_flag, pepper_flag, pad_block, Some(wf.header.wallet_id))?;

    let enc = encrypt_wallet(&secret, pw.as_str(), &hdr)?;
...
fn cmd_build_enc_hint(
    ...
    use pot80_zk_host::keysearch::{KeySearchCtx, AadMode, ValueConceal, tlv, MAX_ENC_HINT_BYTES};
    ...
    ensure!(enc.len() <= MAX_ENC_HINT_BYTES,
        "enc_hint too large: {} bytes (max {})", enc.len(), MAX_ENC_HINT_BYTES);
