...
//! 
//! This module provides verification for 64-bit range proofs using the
//! Bulletproofs protocol with unified Pedersen H generator (cSHAKE256).
//! 
//! **IMPORTANT**: This is the CLASSICAL 2-generator version (G, H).
//! For hybrid PQC commitments (G, H, F), see `hybrid_commit` module.
...
/// Verify 64-bit range proof for commitment V
/// 
/// In True-Trust, V_bytes == C_out (binding enforcement)
pub fn verify_range_proof_64(
    ...
    if lg != proof.ipp.R_vec.len() {
        return Err("L/R len mismatch");
    }
    if lg != log2_exact(N_BITS)? {
        return Err("L/R len != log2(n)");
    }
    ...
    if left != P {
        return Err("ipp check failed");
    }
    if proof.t_hat != ab {
        return Err("t_hat != a*b");
    }
    Ok(())
}

/* ---------- parser + unified H + commit ---------- */
...
/// Parse 64-bit range proof (expected: 672 bytes)
pub fn parse_dalek_range_proof_64(bytes: &[u8]) -> Result<RangeProof, &'static str> {
    ...
    for _ in 0..LG {
        L_vec.push(CompressedRistretto(read32(bytes, &mut off)?));
    }
    for _ in 0..LG {
        R_vec.push(CompressedRistretto(read32(bytes, &mut off)?));
    }
    ...
    Ok(RangeProof {
        ...
        mu,
        t_hat,
        ipp: IppProof { L_vec, R_vec, a, b },
    })
}

/// ⚠️ UNIFIED H PEDERSEN: cSHAKE256("TT-PEDERSEN-H")
/// MUST match hybrid_commit::generator_H()
...
/// Classical Pedersen C(v,r) = r·G + v·H (32B compressed)
...
/// Verify bound range proof (V == C_out binding)
...
/* ---------- NOTE: For hybrid commitments, see hybrid_commit module ---------- */
// Classical Bulletproofs work with 2 generators (G, H).
// For PQC-aware commitments (G, H, F), use hybrid_commit::hybrid_commit().
// The guest ZK still uses classical commitments for speed (~10M cycles).
// PQC binding verified separately in host layer (outside ZK).
