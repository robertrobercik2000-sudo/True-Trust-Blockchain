...
//! GOLDEN TRIO CONSENSUS - Complete Implementation
//!
//! Mathematical model from GOLDEN_TRIO_CONSENSUS.md
//!
//! Formula:
//! ```
...
use crate::pot::{Q, ONE_Q};

/// Hard Trust Metrics (6 components) - NEW GOLDEN TRIO
///
/// Replaces old `QualityMetrics` with measurable, verifiable components
...
pub struct HardTrustMetrics {
    ...
    // T4: Stake Lock
    pub lock_days: u32,
    pub stake_variance: f64,  // Consistency of stake
...
/// Trust component weights (α₁...α₆, suma = 1.0)
#[derive(Clone, Debug)]
...
/// Proof sub-weights (for T2, suma = 1.0)
#[derive(Clone, Debug)]
...
///
/// Formula: lock(t) = 1 + k × ln(1 + t/t_base)
///
...
/// Compute "hard" trust from 6 measurable components
///
...
pub fn compute_hard_trust(
    ...
    let t_stake = (lock_mult / 4.0).min(1.0); // Normalize (max mult ~4.0)
    
    // T5: Fee Collection Trust
    let t_fees = if metrics.expected_fees > 0.0 {
        (metrics.fees_collected / metrics.expected_fees).min(1.0)
    } else { 0.0 };
    
    // T6: Network Participation Trust
...
/// Update trust with decay + reward
///
/// T(t+1) = β·T(t) + (1-β)·T_computed
///
/// Returns: new trust value (Q format)
...
/// Final weight computation (GOLDEN TRIO)
///
/// W_final = T^p_trust × (1+R)^p_randomx × S^p_stake
///
/// Returns: unnormalized weight (to be normalized across all validators)
pub fn compute_final_weight(
    trust: f64,              // T ∈ [0, 1]
    randomx_score: f64,      // R = solved/expected
    stake_fraction: f64,     // S = stake_eff / stake_total
    powers: &PowerParams,
) -> f64 {
...
/// Power parameters for final weight
#[derive(Clone, Debug)]
pub struct PowerParams {
    pub trust: f64,      // p_trust = 1.0 (linear)
    pub randomx: f64,    // p_randomx = 0.5 (sqrt - diminishing returns)
    pub stake: f64,      // p_stake = 0.8 (sub-linear - prevent whales)
}
...
/// Effective stake with time-lock multiplier
///
/// S_eff = stake × lock_multiplier(lock_days)
pub fn compute_effective_stake(stake: u64, lock_days: u32) -> f64 {
    let mult = stake_lock_multiplier(lock_days);
    (stake as f64) * mult
}

/// Minimum stake requirement (scales with network growth)
///
...
/// Slashing amount for violations
///
...
mod tests {
    ...
    fn test_compute_hard_trust_perfect() {
        ...
        // Perfect metrics should give high trust
        assert!(trust > 0.9, "Trust: {}", trust);
    }
    
    #[test]
    fn test_final_weight() {
        let powers = PowerParams::default();
        
        // Alice: high trust, medium CPU, medium stake
        let w_alice = compute_final_weight(0.9, 0.96, 0.4, &powers);
        
        // Bob: low trust, high CPU, low stake
        let w_bob = compute_final_weight(0.4, 1.0, 0.1, &powers);
        
        // Carol: medium trust, low CPU, high stake
        let w_carol = compute_final_weight(0.6, 0.5, 0.7, &powers);
        
        println!("Alice: {:.3}", w_alice);
        println!("Bob: {:.3}", w_bob);
        println!("Carol: {:.3}", w_carol);
        
        // Normalize
        let total = w_alice + w_bob + w_carol;
        println!("Alice%: {:.1}%", 100.0 * w_alice / total);
        println!("Bob%: {:.1}%", 100.0 * w_bob / total);
        println!("Carol%: {:.1}%", 100.0 * w_carol / total);
        
        // Alice should dominate (balanced)
        assert!(w_alice > w_bob);
        assert!(w_alice > w_carol * 0.5);
    }
